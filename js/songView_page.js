...
function setupNavButtons(page, songId, listName) {
  ...
  const transAvail = page.querySelector('#transAvail');
  if (transAvail) {
    transAvail.after(createNavBar());
  } else {
    const content = page.querySelector('.page__content');
    content.prepend(createNavBar());
  }
}
...
function setupFooterNav(page, songId, listName, songList) {
  ...
  const prevDisabled = !prevRec ? 'disabled' : '';
  const nextDisabled = !nextRec ? 'disabled' : '';
